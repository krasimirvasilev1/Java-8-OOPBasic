...
public class CasualRace extends Race {
            ...
            this.getCars().entrySet().stream().sorted((x, y) -> SortByPerformancePoints(x.getValue(), y.getValue())).limit(3)
                    .forEach(x -> {
                        ...
                        builder.append(String.format("%d. %s %s %dPP - $%d%n", this.counter, x.getValue().getBrand(), x.getValue().getModel()
                                , ((x.getValue().getHorsePower() / x.getValue().getAcceleration()) + (x.getValue().getSuspension() + x.getValue().getDurability()))
                                , prizeForParticipant));
    ...
    private int SortByPerformancePoints(Car value, Car value1) {
        int sortByPerformancePoints = Integer.compare(((value1.getHorsePower() / value1.getAcceleration()) +(value1.getSuspension() + value1.getDurability())),((value.getHorsePower() / value.getAcceleration()) +(value.getSuspension() + value.getDurability())));
